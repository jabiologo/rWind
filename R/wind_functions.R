...
#' @return A data.frame or .csv file with U and V vector components for each
...
wind.dl<- function (yyyy,mm,dd,tt,lon1,lon2,lat1,lat2,type="read-data"){
  ...
  if (lon1 > 180 && lon2 <180){
    ...
    if (url.exists(url_west)== FALSE | url.exists(url_east)== FALSE){stop("rWind cannot connect with the server. It could be due to the following issues:
    -You have not access to internet conexion.
    -Area, dates or time provided are not available.
    -Server is down.")}
    ...
    if (url.exists(url_dir)== FALSE){stop("rWind cannot connect with the server. It could be due to the following issues:
    -You have not access to internet conexion.
    -Area, dates or time provided are not available.
    -Server is down.")}
    if (type == "csv"){
...
#' in GIS (Geographic Information Systems) or R plotting. Moreover, it cleans
...
#' @return A data.frame prepared to be plotted by R or to be exported as a CSV
#' file to be used with a GIS software.
...
#' by raster file.
#' @return A raster file representing wind direction or wind speed of the study
#' area.
#' @author Javier Fernández-López (jflopez@@rjb.csic.es)
...
#' @rdname wind2raster
#' @export wind2raster
wind2raster<- function(x, type="dir"){
...
#' @references "R Package shape: functions for plotting graphical shapes,
#' colors..." Karline Soetaert
...
#'
#' @param dl Direction layer. Raster layer obtained from wind2raster function
#' ("rWind" package) with flow direction values.
#' @param sl Speed layer. Raster layer obtained from wind2raster function
...
#' @return A TransitionLayer object from "gdistance" package that could be used
...
#' cluster.
#' @author Javier Fernández-López (jflopez@@rjb.csic.es); Jacob van Etten
...
#' @rdname wind2raster
#' @export
sea2raster<-function(x, type="dir"){
  coordinates(x) <- ~lon+lat
  x$speed[is.nan(x$speed)==TRUE] <- -9999
  x$dir[is.nan(x$dir)==TRUE] <- -9999
  rast <- raster(ext=extent(x), nrows=length(x$lon[x$lon == x$lon[1]]) , ncols=length(x$lat[x$lat == x$lat[1]]))

  if (type == "dir"){
    r<-rasterize(x, rast, x$dir) }
  else {
    r<-rasterize(x, rast, x$speed)
  }
  r[r[]==-9999]<-NA
  return(r)
}
